...
/*作用是维持data to device以及data from device两个stream，以支持cuda的异步memcpy，同时内涵了一个kernel stream，实际是为应用中的核函数运算提供一个专门的stream，将data transfer与compute分离，overlap开销*/
class StreamManager {
public:
...
/* Host global */
/*全局变量，使用时依靠stream_mngr与gpu通信。需要先创建一个hostcache，然后在核函数中显式地指定对应地计算流*/
...
uint32_t which_tier_to_evict(PageReuseTable* prt, uint64_t curr_virt_timestamp, bool& reuse_place_decision, bool dirty = false)
{
    ...
    /*表示这个页还没有被驱逐过，这时候如果是脏页的话先放在host memory上，估计是放在host memory上再选中他的时候，写回ssd，因为如果是脏页的话
    他肯定先被加载到gpu上了，如果不是脏页那么放哪其实无所谓了*/
    ...
    */
   /*如果是最后一次运算的话，那么就直接把这个页写入到ssd就好，这个其实需要在应用里显式地使用，算是一个比较ugly的方法*/
    if (last_iteration) {
        #if GET_GOLDEN_REUSE_DISTANCE
    ...
    /*表示接下来的逐出策略是基于reuse策略，这个需要在page cache里的find slot函数中用到*/
...
/*初始化，创建host上的cache，此函数作为一个参数传递给createHostCache中的pthread_create，调用一个线程执行此函数*/
static void* start_host_runtime_thread(void* arg) 
{
