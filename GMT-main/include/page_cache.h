...
//chenjiahao 是否使用mem管理，即利用reuse distance来管理page
#define ENABLE_TMM 1
...
/*page_cache_t 一般作为h_pc 被初始化，就是一个大块的页缓存管理器，里面有一个指向page_cache_d_t的指针pdt，用来链接gpu上的页缓存管理器*/
struct page_cache_t {
...
uint32_t page_cache_d_t::find_slot(uint64_t address, uint64_t range_id, const uint32_t queue_, int32_t* bid) {
    ...
    // 将 range_id 和 address 合并为逻辑页面的唯一全局标识符。
    ...
    do {
        /* 这里使用了一个原子计数器 page_ticket。每个调用 find_slot 的线程（尤其是在 GPU 这种并行环境下）会原子地增加
        这个计数器并获取一个值，然后对总页数 n_pages 取模。
        这是一种简单的策略，让不同的线程从不同的槽位开始查找，减少冲突，实现一种近似Round-Robin探测方式*/
        page = page_ticket->fetch_add(1, simt::memory_order_relaxed)  % (this->n_pages);
        bool lock = false;

        /*拿到这个物理槽位的状态，分为三种情况：
        1. free：证明这个槽位处于空闲状态，其上的page被已经被驱逐或者是还没使用过。
        2. unlocked：表示这个槽位有内容，但是没有线程在使用这个槽位。
        3. locked: 表示有线程在对这个槽位进行专有的操作，比如说正在驱逐或是写入page*/
